...
// LessThan implements an ordering of keys. Invalid keys (deleted keys, etc.)
// have an arbitrary ordering. The ordering matches that generated by [Keys].
...
// Index returns an index, between 0 and [Store.Count] minus one inclusive,
// indicating, in ascending order, where a key would appear in the iteration
// generated by [Keys].
//
// Panics with ErrNotFound if the key is no longer valid.
func (s *Store[ValueT]) Index(key Key) int {
    _, ok := lookup(s, key)
    if !ok { panic(ErrNotFound) }
