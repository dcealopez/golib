...
func (i IconType) iconFlag() uint32 {
	switch i {
	case IconInfo:
		return windows.MB_ICONINFORMATION
	case IconWarning:
		return windows.MB_ICONWARNING
	case IconError:
		return windows.MB_ICONERROR
	}
...
func (m FilePicker) pick(
	...
	filters.WriteByte(0)
	titlePtr, err := syscall.UTF16PtrFromString(m.Title)
...
func (m Message) flags(message string) uint32 {
	...
	for _, r := range message {
		...
		switch cls {
		case bidi.R:
			isRtl = true // Strong R-to-L
		case bidi.AL:
			isRtl = true // Strong R-to-L
		case bidi.RLO:
			isRtl = true // Explicit R-to-L
		}
		if isRtl {
